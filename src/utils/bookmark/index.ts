...
import { getRecentHistory, IDB_PREFIX_HISTORY_RECENT } from "utils/history";
import { siteMetadata, isWEBP, novelsData } from "store/states";
...
export function loadBookmark(): Promise<Bookmark[]> {
  ...
  return new Promise((resolve, reject) => {
    getBookmarkDataAll().then((list) => {
      if (list.length && (process as any).browser) {
        resolve(list);
      } else {
        reject();
      }
